classdef hamish_IBVS < handle
    ...
    properties
        ...
        points;
        initialDepth;
        originalStart;           % Original starting position
        acceptableError = 10;    % Acceptable error threshold
        fig;                     % Figure for displaying results
        avgPosition;             % Average position of detected points
        originalZ;               % Original depth value
        z_cm_values;             % Array to store depth in centimeters
        y_cm_values;             % Array to store Y positions in centimeters
        x_cm_values;             % Array to store X positions in centimeters
        z_values;     
        x_cm;                    % X position in centimeters
        y_cm;                    % Y position in centimeters
        zCam_cm;                 % Depth from the camera in centimeters
        GUI;
...
%% Camera Calibration Data and setting varaibles
            ...
            s = [0, 27.5];
            t = .5;               % Define the scalar factor t (you may need to adjust this, (3.5 works for the phone,1.7 for the big board) )

            self.z_cm_values = [];                                
            self.x_cm_values = [];
            self.y_cm_values = [];

            a = self.points(1, :);  % Choose a reference point                                   
            prevZ = 0;

%% Testing for 3 Points anywhere on checkerboard
            % Finding two points (b and c) to form a triangle with the reference point
...
%% Main Function to run body of code
        function main(self)
            ...
            acceptableErrorX = 6.0;                                                         % Adjust as needed
            acceptableErrorY = 6.0;                                                         % Adjust as needed
            acceptableErrorZ = 30.0;                                                        % Adjust as needed
% Create a camera object using Hamish's laptop's front-facing camera when needed
            vid = webcam("Integrated Camera"); %Using Sarah's laptop camera
