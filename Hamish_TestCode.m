...
while ishandle(fig)
    ...
    if ~isCalibrationStarted
        ...
        imshow(img);
        text(20, 20, 'Press space to start', 'Color', 'red', 'FontSize', 16);
        
        % Wait for spacebar input to start calibration
        k = waitforbuttonpress;
        if k == 1 && strcmp(get(fig,'CurrentKey'), 'space')
            % Reset original start position
            originalStart = [];
    ...
    if ~isempty(imagePoints)
        ...
        for i = 2:size(points, 1)
            ...
            x_cm = (fx * deltaX) / fx / 10; % Convert pixel to cm for X-axis (assuming a square pixel)
            y_cm = (fy * deltaY) / fy / 10; % Convert pixel to cm for Y-axis (assuming a square pixel)
